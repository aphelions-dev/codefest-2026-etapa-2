...
# Solo Colombia tiene geometria departamental cargada; el resto de la cuenca se lista sin mapa.
MAPPED_COUNTRY = "Colombia"
...
async def by_territory(pool: asyncpg.Pool, group: str | None) -> list[asyncpg.Record]:
    """Departamentos con su geometria y cuantos municipios suyos registran presencia."""
    filtered = "and m.groups @> array[$1::text]" if group else ""
    ...
    async with pool.acquire() as connection:
        return await connection.fetch(
            ...
            with per_admin1 as (
                ...
                where m.country = '{MAPPED_COUNTRY}' {filtered}
                group by 1
            )
            select p.place_id, p.name, p.iso2, p.geometry,
                   a.municipalities, a.with_presence, a.without_information, a.groups,
                   a.sample_doc, a.sample_chunk
            from per_admin1 a
            join places p on p.level = 'department'
                 and {name} = {admin}
            order by a.with_presence desc
            """,
            *args,
        )
...
async def municipalities(
    pool: asyncpg.Pool, group: str | None, country: str | None, limit: int
) -> list[asyncpg.Record]:
